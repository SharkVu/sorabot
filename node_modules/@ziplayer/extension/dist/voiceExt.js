...
class voiceExt extends ziplayer_1.BaseExtension {
    // private _activeSpeakers = new Set<string>();
    constructor(player = null, opts) {
        super();
        this.name = "voiceExt";
        this.version = "1.0.0";
        this.player = null;
    ...
    active(alas) {
        if (alas?.player && !this.player)
            this.player = alas.player;
        const player = this.player;
        const manager = alas?.manager;
        if (manager)
    ...
    attach(client, opts) {
        if (client)
            this.client = client;
        if (opts)
            this.speechOptions = { ...this.speechOptions, ...opts };
        const connection = this.player?.connection;
        ...
        this.player.on("playerDestroy", () => {
            try {
                const conn = this.player?.connection;
    ...
    handleSpeakingEvent() {
        ...
        speaking.on("start", (userId) => {
            ...
            const pendingOverrides = Promise.resolve(this.speechOptions?.onVoiceChange?.({
                userId,
                channelId,
                guildId,
                player: this.player,
            ...
            opusStream
                ...
                const effective = { ...this.speechOptions, ...overrides };
    ...
    async processVoice(userId, bufferData, effective) {
        ...
        try {
            ...
            if (this.manager && typeof this.manager.emit === "function") {
                this.manager.emit("voiceCreate", this.player, payload);
            }
            else {
                this.player?.emit?.("voiceCreate", payload);
            }
        }
        catch (err) {
    ...
    async resolveSpeech(audioBuffer, opts) {
    ...
    async resolveSpeechWithGoogleSpeechV2(audioBuffer, opts) {
        const use = opts ?? this.speechOptions;
    ...
    resolveGoogleSpeechKey(use) {
        ...
        this.debug("No GSPEECH_V2_KEY provided — using a shared fallback key (dev/testing). Set your own key to control quota. See https://cloud.google.com/speech-to-text/v2/docs/recognizers");
        return fallback;
    }
...
exports.voiceExt = voiceExt;
//# sourceMappingURL=voiceExt.js.map